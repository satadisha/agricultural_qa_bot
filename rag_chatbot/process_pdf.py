...
def truncate_text(text: str, max_tokens: int = MAX_TOKENS) -> str:
    """Truncate text so it does not exceed a given token limit."""
    input_ids = tokenizer.encode(text, add_special_tokens=False)[:max_tokens]
    return tokenizer.decode(input_ids, skip_special_tokens=True)


def detect_language(text: str) -> str:
    """Detect the language code for the given text, or 'unknown' on failure."""
    try:
        return detect(text)
    except Exception:
        return "unknown"
...
def fingerprint(text: str) -> str:
    """Return a simple normalized fingerprint using the first 50 words."""
    return " ".join(text.lower().split()[:50])


def process_single_pdf(pdf_path: Path, jsonl_dir: Path, chroma_path: Path):
    """Process one PDF: extract, section, chunk, embed, save JSONL, and store in Chroma."""
...
def main():
    """CLI entry point for processing one or many PDF files into JSONL + Chroma."""
