...
def keyword_overlap_score(text: str, query: str) -> int:
    """Return how many important keywords appear in both the text and the query."""
...
def extract_article_number_from_query(query: str) -> str:
    """Extract 'article N' from the query if it exists; otherwise return None."""
...
def rerank_chunks(results, query_embedding, query_text):
    """
    Rerank chunks using cosine similarity plus keyword and article-number bonuses.
    Returns a list of tuples sorted by final score.
    """
...
def get_top_chunks(country, query, chroma_path, top_k=10):
    """
    Query the regulations collection for a specific country, rerank results,
